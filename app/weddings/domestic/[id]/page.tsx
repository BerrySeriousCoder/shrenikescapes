...
import itinerariesData from "../../../../lib/data/domestic_wedding_itineraries.json";
import { use } from "react";

export default function DomesticWeddingDetail({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const wedding = weddings.find(w => w.id === resolvedParams.id && w.region === "domestic");
  
  if (!wedding) return notFound();

  // Try to get actual parsed itinerary from the json, otherwise fallback
  // Extract just the name matching the json object key. e.g "goa-wedding" -> "goa"
  const locationKey = wedding.id.split('-')[0] as keyof typeof itinerariesData;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const itinerary = (itinerariesData as any)[locationKey] || [];
         ...
         <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row gap-12 items-center">
            <div className="w-full md:w-1/2">
               ...
               <p className="text-lg md:text-xl text-white/70 font-light leading-relaxed mb-8">
                 Every detail of your {wedding.duration} domestic celebration is meticulously crafted to ensure absolute luxury and flawless memories. From breathtaking venues to unparalleled hospitality, your wedding journey is designed to perfection.
               </p>
            </div>
