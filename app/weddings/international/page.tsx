...
export default function InternationalWeddings() {
  const intlWeddings = weddings.filter((w: Wedding) => w.region === "international");
    ...
    <div className="bg-[var(--color-brand-cream)] min-h-screen font-sans">
      ...
      <div className="relative h-[70vh] w-full pt-24 mb-12">
         <Image 
           ...
           alt="International Weddings Hero"
           fill
           priority
           className="object-cover"
         />
         <div className="absolute inset-0 bg-gradient-to-b from-[#1c305a]/80 via-[#1c305a]/40 to-[var(--color-brand-cream)]"></div>
         ...
         <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 z-10 pt-20">
           ...
           >
             <span className="text-[var(--color-brand-yellow)] font-bold tracking-widest uppercase text-sm">Global Venues</span>
           </motion.div>
           <h1 className="text-6xl md:text-8xl lg:text-9xl font-bold text-[var(--color-brand-cream)] mb-6 tracking-tighter mix-blend-overlay">
             A World of <br /> Romance
           </h1>
           <p className="text-xl md:text-3xl text-[var(--color-brand-cream)]/90 max-w-3xl font-light">
             Say &quot;I Do&quot; at the most stunning and exotic destinations on earth.
           </p>
         </div>
      </div>
      ...
      <section className="pb-32 px-4 md:px-8 max-w-[1400px] mx-auto w-full">
        <div className="flex flex-col gap-4 md:gap-8">
          {intlWeddings.map((wedding: Wedding, index: number) => {
            const isHovered = hoveredId === wedding.id;
            
            return (
              <Link 
                key={wedding.id} 
                href={`/weddings/international/${wedding.id}`} 
                className="group relative w-full rounded-[2rem] overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] flex cursor-pointer shadow-xl border border-[#1c305a]/10"
              ...
              >
                ...
                {/* Gradient Overlays */}
                <div className="absolute inset-0 bg-gradient-to-r from-[#111c34]/90 via-[#1c305a]/50 to-transparent transition-opacity duration-500"></div>
                ...
                <div className="relative z-10 w-full h-full p-8 md:p-14 flex flex-col justify-between">
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between w-full">
                    ...
                    <div className="mt-8 md:mt-0 flex items-center gap-6">
                      ...
                      {/* FAB Icon */}
                      <div className="w-16 h-16 rounded-full bg-[var(--color-brand-cream)] flex items-center justify-center text-[#1c305a] transform transition-transform duration-500 group-hover:rotate-45 group-hover:bg-[#F3C10A] group-hover:text-[#1c305a]">
