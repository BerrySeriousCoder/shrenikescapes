...
export default function DomesticTours() {
  const domesticTours = tours.filter((t: Tour) => t.region === "domestic");
    ...
    <div className="bg-[var(--color-brand-cream)] min-h-screen font-sans">
      ...
      <div className="relative h-[70vh] w-full pt-24 mb-12">
         <Image 
           ...
           alt="Domestic Tours Hero"
           fill
           priority
           className="object-cover"
         />
         <div className="absolute inset-0 bg-gradient-to-b from-[#0d2613]/80 via-[#0d2613]/40 to-[var(--color-brand-cream)]"></div>
         ...
         <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 z-10 pt-20">
           ...
           >
             <span className="text-[var(--color-brand-yellow)] font-bold tracking-widest uppercase text-sm">The Curated Collection</span>
           </motion.div>
           ...
           <p className="text-xl md:text-3xl text-[var(--color-brand-cream)]/90 max-w-3xl font-light">
             Cinematic journeys across the homeland, designed for the extraordinary.
           </p>
         </div>
      </div>
      ...
      <section className="pb-32 px-4 md:px-8 max-w-[1400px] mx-auto w-full">
        <div className="flex flex-col gap-4 md:gap-8">
          {domesticTours.map((tour: Tour, index: number) => {
            const isHovered = hoveredId === tour.id;
            
            return (
              <Link 
                key={tour.id} 
                href={`/tours/domestic/${tour.id}`} 
              ...
              >
                ...
                {/* Gradient Overlays */}
                <div className="absolute inset-0 bg-gradient-to-r from-[#0d2613]/90 via-[#0d2613]/50 to-transparent transition-opacity duration-500"></div>
                ...
                <div className="relative z-10 w-full h-full p-8 md:p-14 flex flex-col justify-between">
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between w-full">
                    <div className="max-w-xl">
                      <h3 className="text-5xl md:text-7xl lg:text-8xl font-bold text-[var(--color-brand-cream)] tracking-tighter mb-2 transition-transform duration-500 transform origin-left group-hover:scale-105 whitespace-nowrap">
                        {tour.title}
                      </h3>
                    ...
                    <div className="mt-8 md:mt-0 flex items-center justify-between w-full md:w-auto gap-6">
                      ...
                      {/* FAB Icon */}
                      <div className="w-16 h-16 rounded-full bg-[var(--color-brand-cream)] flex items-center justify-center text-[var(--color-brand-green)] transform transition-transform duration-500 group-hover:rotate-45 group-hover:bg-[var(--color-brand-blue)] group-hover:text-white shrink-0">
